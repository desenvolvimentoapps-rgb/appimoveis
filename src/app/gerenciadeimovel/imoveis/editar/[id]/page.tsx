...
export default function EditarImovelGerenciaPage() {
    ...
    if (loading) return (
        <div style={{ padding: "10rem", textAlign: "center" }}>
            ...
            <p style={{ marginTop: "1.5rem", fontWeight: 700, color: "var(--text-muted)" }}>Sincronizando portal corporativo...</p>
        </div>
    );
    ...
    return (
        <form onSubmit={handleSubmit} style={{ maxWidth: "1200px", margin: "0 auto", paddingBottom: "10rem" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "3rem" }}>
                <div>
                    <h2 style={{ fontSize: "2.25rem", color: "var(--color-dark-blue)", fontWeight: 900, letterSpacing: "-1px" }}>Gestão de Imóvel</h2>
                    <p style={{ color: "var(--text-muted)", fontSize: "1rem", fontWeight: 500 }}>Refine os detalhes técnicos e visuais da sua listagem.</p>
                </div>
                <div style={{ display: "flex", gap: "1.25rem" }}>
                    ...
                    <button type="submit" className="btn btn-primary" style={{ padding: "0.8rem 2.5rem", borderRadius: "12px", fontWeight: 800, backgroundImage: "linear-gradient(135deg, var(--color-dark-blue) 0%, #2c3e50 100%)", boxShadow: "0 10px 20px rgba(0,0,0,0.1)" }}>Salvar Alterações</button>
                </div>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 350px", gap: "2.5rem" }}>
                <div style={{ display: "flex", flexDirection: "column", gap: "3rem" }}>
                    ...
                    <section className="card" style={{ padding: "2.5rem", borderRadius: "24px" }}>
                        ...
                        <div className="form-group" style={{ marginTop: "1.5rem" }}>
                            <label style={labelStyle}>Narrativa do Imóvel</label>
                    ...
                    {selectedType && selectedType.customFields?.length > 0 && (
                        <section className="card" style={{ padding: "2.5rem", borderRadius: "24px", borderLeft: "6px solid var(--accent-secondary)" }}>
                            <h3 style={{ marginBottom: "2rem", color: "var(--accent-secondary)", fontWeight: 900, fontSize: "1.25rem" }}>✨ Diferenciais Específicos</h3>
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
                    ...
                    <section className="card" style={{ padding: "2.5rem", borderRadius: "24px" }}>
                        ...
                        <div
                            ...
                            style={{ border: "2px dashed #cbd5e1", padding: "4rem 2rem", borderRadius: "20px", textAlign: "center", backgroundColor: "#f8fafc", cursor: "pointer" }}
                        >
                            <input type="file" multiple accept="image/*" ref={fileInputRef} onChange={handleFiles} style={{ display: "none" }} />
                            <div style={{ fontSize: "2.5rem", marginBottom: "1rem" }}>📸</div>
                            <p style={{ fontWeight: 700, color: "var(--color-dark-blue)" }}>Atualizar Fotografias</p>
                        </div>
                        {images.length > 0 && (
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))", gap: "1.25rem", marginTop: "2.5rem" }}>
                                {images.map((img, idx) => (
                                    <div key={idx} style={{ position: "relative", height: "150px", borderRadius: "15px", overflow: "hidden", border: "2px solid #eef2f6" }}>
                                        <img src={img} style={{ width: "100%", height: "100%", objectFit: "cover" }} alt="Vista" />
                                        <button type="button" onClick={() => setImages(images.filter((_, i) => i !== idx))} style={{ position: "absolute", top: "8px", right: "8px", backgroundColor: "rgba(239, 68, 68, 0.9)", color: "white", border: "none", borderRadius: "8px", width: "28px", height: "28px", cursor: "pointer" }}>✕</button>
                                    </div>
                                ))}
                            </div>
                        )}
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "2.5rem", position: "sticky", top: "2rem", height: "fit-content" }}>
                    <section className="card" style={{ padding: "2rem", borderRadius: "24px" }}>
                        <h4 style={{ marginBottom: "1.5rem", fontWeight: 800, color: "var(--color-dark-blue)" }}>Disponibilidade</h4>
                    ...
                    <section className="card" style={{ padding: "2rem", borderRadius: "24px" }}>
                        ...
                        <div style={{ maxHeight: "400px", overflowY: "auto", display: "flex", flexDirection: "column", gap: "0.75rem" }} className="no-scrollbar">
                            {comodidadesDisponiveis.map(c => (
                                <label key={c.id} style={{ display: "flex", alignItems: "center", gap: "0.75rem", fontSize: "0.9rem", cursor: "pointer", padding: "0.75rem", borderRadius: "12px", border: "1px solid #f1f5f9" }}>
                                    <input
                                        type="checkbox"
